...
import type { Transactions, EcashTransaction, LightningTransaction, WalletTransaction, Wallet } from '@fedimint/core-web';
...
>(
    ...
    async ({ limit, lastSeen, wallet }) => {
        ...
        const formattedTransactions: Transaction[] = await Promise.all(
            rawTxs.map(async (tx: Transactions) => {
                ...
                if (tx.kind === 'ln') {
                    ...
                    outcome = (tx as LightningTransaction).outcome ?? null;
                    ...
                    outcome = (tx as EcashTransaction).outcome ?? null;
                } else if (tx.kind === 'wallet') {
                    amountMsats = (tx as WalletTransaction).amountMsats;
                    outcome = (tx as WalletTransaction).outcome ?? null;
        ...
        const newLastSeen = rawTxs.length > 0
            ? {
                creation_time: {
                    secs_since_epoch: Math.floor(new Date(rawTxs[rawTxs.length - 1].timestamp).getTime() / 1000),
                    nanos_since_epoch: (new Date(rawTxs[rawTxs.length - 1].timestamp).getTime() % 1000) * 1_000_000,
                },
        ...
        builder
            ...
            .addCase(fetchTransactions.fulfilled, (state, action) => {
                if (state.isSearchMode) {
                    ...
                    state.transactions = [...state.transactions, ...action.payload.formattedTransactions];
                }
