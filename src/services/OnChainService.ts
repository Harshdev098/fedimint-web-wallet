...
export const subscribeDeposit = (wallet: Wallet, operationId: string, dispatch: AppDispatch,setDepositState:React.Dispatch<string | null>) => {
    const unsubscribe = wallet.wallet.subscribeDeposit(operationId,
        (state: WalletDepositState) => {
            ...
            if (state === 'WaitingForTransaction') {
                ...
                dispatch(createNotification({ type: 'Onchain', data: 'Waiting for confirmation', date, time, OperationId: operationId }))
            } else if (typeof state === 'object' && 'Confirmed' in state) {
                setDepositState('confirmed')
                dispatch(createNotification({ type: 'Onchain', data: 'Payment confirmed', date, time, OperationId: operationId }))
            } else if (typeof state === 'object' && 'Claimed' in state) {
                setDepositState('claimed')
                dispatch(createNotification({ type: 'Onchain', data: 'Payment claimed', date, time, OperationId: operationId }))
            } else if (typeof state === 'object' && 'Failed' in state) {
                setDepositState('failed')
                dispatch(createNotification({ type: 'Onchain', data: 'Payment Failed', date, time, OperationId: operationId }))
            }
        },
        (error: any) => {
