...
export async function handleZapRequest(
    ...
    try {
        ...
        if (!pTag || !pTag[1]) {
            return {
                error: { code: 'INVALID_ZAP_REQUEST', message: "Missing or invalid 'p' tag" },
            };
        }
        if (!amountTag || !amountTag[1]) {
            return {
                error: { code: 'INVALID_ZAP_REQUEST', message: "Missing or invalid 'amount' tag" },
            };
        }
        if (!lnurlTag || !lnurlTag[1]) {
            return {
                error: { code: 'INVALID_ZAP_REQUEST', message: "Missing or invalid 'lnurl' tag" },
            };
        }
        ...
        try {
            ...
            if (!invoice) {
                return {
                    error: { code: 'LNURL_ERROR', message: 'Failed to fetch invoice from LNURL' },
                };
            }
        } catch (error) {
        ...
        return {
            error: { code: 'ZAP_PROCESSING_ERROR', message: 'Failed to process zap request' },
        };
    }
}
