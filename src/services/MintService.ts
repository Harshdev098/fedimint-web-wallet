...
export const NoteCountByDenomination = async (
    wallet: Wallet
): Promise<NotesByDenomonationResponse> => {
    try {
        ...
        (state: SpendNotesState) => {
            ...
            if (state === 'UserCanceledProcessing') {
                dispatch(
                    createNotification({
                        type: 'ecash',
                        data: 'Ecash Notes Cancelled processing',
                        date: date,
                        time: time,
                ...
                dispatch(
                    createNotification({
                        type: 'ecash',
                        data: 'Ecash Notes Cancelled success',
                        date: date,
                        time: time,
                        OperationId: operationId,
                    })
                ...
                dispatch(
                    createNotification({
                        type: 'ecash',
                        data: 'Ecash Notes Cancelled failed',
                        date: date,
                        time: time,
                        OperationId: operationId,
                    })
