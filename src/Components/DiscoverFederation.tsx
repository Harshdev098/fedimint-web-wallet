...
export default function DiscoverFederation({ setShowFederation, showFederations, joinFederation, setRecover,recover}: DiscoverFederationProps) {
    ...
    return (
        <section className='modalOverlay'>
            <div className='previewCard'>
                <button type='button' className='closeBtn' onClick={() => setShowFederation(false)}>
                                ...
                                <div className="fed-elements-details">
                                    ...
                                    {expandedFed === fed.federationID && (
                                        <div className="federation-details">
                                            <div>
                                                <p><b>Guardians:</b> {fed.totalGuardians}</p>
                                                <p><b>Max stable Balance:</b> {fed.maxBalance}</p>
                                                <p><b>Message:</b> {fed.welcomeMessage}</p>
                                                <p><b>Onchain deposit:</b> {fed.onChainDeposit === 'true' ? 'Disabled' : 'Enabled'}</p>
                                                <p><b>Services(modules):</b> {fed.modules && Object.values(fed.modules).length > 0
